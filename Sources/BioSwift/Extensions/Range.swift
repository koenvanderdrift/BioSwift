...
public typealias DoubleChainRange = ClosedRange<Double>
...
public extension ChainRange {
    ...
    var toDoubleRange: DoubleChainRange {
        guard isValidChainRange else {
            return 0.0...0.0
        }
...
public extension DoubleChainRange {
    func toChainRange(
        ...
        let roundedLower = lowerBound.rounded(roundingRule)
        let roundedUpper = upperBound.rounded(roundingRule)

        guard let lower = Int(exactly: roundedLower),
              let upper = Int(exactly: roundedUpper),
              lower > 0,
              upper >= lower
        else {
            return zeroChainRange
    ...
    /*
     Useful when a continuous interval should include every touched
     character position.
     ...
     */
    func toChainRange(
        lowerRoundingRule: FloatingPointRoundingRule,
        upperRoundingRule: FloatingPointRoundingRule
    ) -> ChainRange {
        guard lowerBound.isFinite,
              upperBound.isFinite
        else {
        ...
        let roundedLower = lowerBound.rounded(lowerRoundingRule)
        let roundedUpper = upperBound.rounded(upperRoundingRule)
